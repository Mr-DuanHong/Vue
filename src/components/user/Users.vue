<template>
  <div>
    ...
    <el-card class="box-card">
      <!--搜索与添加区,栅格系统，:gutter表示间距20，:span是比例-->
      <el-row :gutter="20">
        <el-col :span="7">
...
<script>export default {
  ...
  methods: {
    async getUserList () { // 发起axios数据请求得到用户数据
      const { data: res } = await this.$http.get('users', { params: this.queryInfo })
      if (res.meta.status !== 200) {
        return this.$message.error('获取用户列表失败！')
      }
      this.userlist = res.data.users // 将得到的用户数据加入到网页的数据列表中
    ...
    // 监听添加用户对话框的关闭事件,使得每次关闭对话框再重新打开就是新的
    addDialogClosed () {
    ...
    addUser () {
      this.$refs.addFormRef.validate(async valid => {
        if (!valid) return
        // 当为false，返回出去。为true则发起添加用户的网络请求
